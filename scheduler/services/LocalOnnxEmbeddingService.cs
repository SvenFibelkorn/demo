...
public sealed class LocalOnnxEmbeddingService : IEmbeddingGenerator<string, Embedding<float>>, IDisposable
{
    ...
    {
        var inputIds = _tokenizer.EncodeToIds(
            text,
        ...
        var inputIdsTensor = BuildInputTensor(inputIds, _maxLength, _tokenizer.PaddingTokenId);
        ...
        AddInput(inputs, "input_ids", inputIdsTensor.tensor, required: true);
        AddInput(inputs, "attention_mask", attentionMask.tensor, required: false);
        AddInput(inputs, "token_type_ids", tokenTypeIds.tensor, required: false);
